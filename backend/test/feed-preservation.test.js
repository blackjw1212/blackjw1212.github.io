...
const CODES = ["2330", "2317", "6669", "3017", "3324", "2382", "1519", "2308", "3231", "3661"];
...
test("total upstream failure keeps all previous eod/valuation/yield", () => {
  ...
  assert.equal(merged.eod.length, 10);
  assert.equal(Object.keys(merged.valuation).length, 10);
  ...
  assert.equal(merged.preserved.eod, 10);
  assert.equal(merged.preserved.valuation, 10);
...
test("partial fetch overlays fresh and preserves the rest", () => {
  ...
  assert.equal(merged.eod.length, 10, "all codes retained");
  ...
  assert.equal(merged.preserved.eod, 9);
  assert.equal(merged.preserved.valuation, 9);
