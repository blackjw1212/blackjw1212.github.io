...
- Added `/forscan/sync3/` — a Syn3Updater (CyanLabs) SYNC 3 update guide: requirements (SYNC 3.0 not 2.5, exFAT USB 8GB non-nav / 32GB nav, Windows, 30-60 min), the three install modes (AutoInstall / Reformat / Downgrade) with pros and cons, MY20 detection via the Interrogator Log, illustrated step order, and Utility/troubleshooting notes. Leads with the irreversible-upgrade warning (1.x/2.x/3.0 → 3.2+ cannot go back to 3.0) and links to the CyanLabs sources. Wired into the main page, sw.js precache and the static-site contract.
- Split all maintenance content into a dedicated sub-page `/forscan/service/` (parts list, torque, oil / transmission / air-filter / cabin-filter illustrated step guides, full DIY steps). The main `/forscan/` page now keeps the FORScan configuration categories and links to the sub-page. Each illustrated figure uses an inline SVG placeholder that an `<img>` (under `forscan/service/img/`) auto-replaces when a real photo is dropped in (documented in that folder's README).
- Added a 維修圖解 · 機油更換 section styled like the OEM manual: numbered oil-change / oil-level steps with ※torque callouts and a 2x2 grid of labelled inline-SVG component diagrams (加油孔 / 量油尺 / 機油濾芯 / 洩油孔). Corrected torque to the official Ford Lio Ho manual values: oil filter 14 Nm, drain plug 27 Nm (previously ~28 Nm/hand-turn estimates), propagated across the maintenance cards and DIY steps.
