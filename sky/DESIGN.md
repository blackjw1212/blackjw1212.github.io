...
## 版面：滿版取景器 + 右上角半透明面板（2026-09-09）

原本是「上面 62vh 相機 + 下面一份說明文件」。那是文件頁的形狀，不是取景器的形狀：
實際使用時人是舉著手機對天空，眼睛全程在畫面上，而畫面只佔 62%、控制項全在看不到的
下方。改成相機佔滿視窗、操作中真的會看的東西（視野角、星等、指向與定位狀態）收進
右上角一塊半透明 `<details>`，說明與星表往下捲。

- **不走 Fullscreen API。** iPhone 的 Safari 至今不支援對一般元素呼叫
  `requestFullscreen()`（只有 `<video>` 可以），而這頁的主要裝置就是手機。
  頁面本來就有 `viewport-fit=cover`，CSS 滿版（`100dvh`）配 safe-area 是這裡唯一
  到得了的答案。`dvh` 讓網址列收合／展開時高度跟著走。
- **`touch-action` 一定要跟著改，這是滿版帶來的新坑。** 畫布原本完全禁用觸控手勢；
  舞台只佔 62vh 時還有別處可以拖著捲，改成滿版之後整個第一屏都是那張畫布。
  **實測（CDP、375×812、模擬觸控）：手指從畫布上拖曳，`scrollY` 從 0 到 0 —— 整頁
  捲不動，而且沒有任何徵兆。** 改成 `manipulation` 之後同樣的拖曳得到 `scrollY=439`。
  畫布只處理點擊，本來就不需要吞掉平移。靜態契約釘住這件事。
- **`env(safe-area-inset-*)` 是必要的，不是講究。** `viewport-fit=cover` 之下沒有它，
  面板會鑽進瀏海／狀態列底下，而**桌機模擬的 inset 全是 0、完全看不出來**。
  同樣由靜態契約釘住。
- **開始區靠下對齊，不是置中。** 面板最高會壓到畫面中段，置中的文字會鑽到它底下
  ——實測 500×725 下提示文字 y=295 與面板的 10〜328 正面對撞。放在下緣同時也是
  拇指構得到的位置。面板另外有 `max-height` + 自捲，矮螢幕時不會吃掉整個畫面。
- **開始區鋪滿整個舞台，所以預設 `pointer-events:none`、只有按鈕是 `auto`。**
  有 blocker 時它會在 running 中繼續顯示，若不放行就會把畫布的點擊全部吃掉
  ——點畫面問「那裡是什麼」當場失效，而且不會報錯。
  規則是「dock 可見 ⇔ 還有話要說」，錯誤路徑（`start()` 與 `frame()` 兩個 catch）
  都要把它叫回來，否則錯誤訊息會寫進一個看不到的元素。
- **`FOV_STORAGE_KEY` 換成 `-v2`。** video 是 `object-fit:cover`，那是**裁切**；
  舞台長寬比從約 0.77 變成 0.46 之後，同一支鏡頭在畫面上呈現的水平視野角就不一樣了，
  舊的校正值在新版面下是錯的。讓它退回誠實的預設 65°（旁邊本來就寫著要自己校正），
  比沿用一個在別的裁切下量到的數字好。
- **`sw.js` 的 `VERSION` 不用 bump。** `/sky/` 是導覽請求，走 network-first；
  `PRECACHE` 裡的那份只在離線時才端出來。（要 bump 的是 cache-first 的那些資產，
  例如 `/subtitle/` 的 worker 與 vendor。）

模擬量到的（CDP + 環境自帶的 Chromium，`pointer:coarse`、`deviceScaleFactor:2`）：

| | 375×812 | 320×720 |
|---|---|---|
| 舞台 | 375×812（滿版） | 320×720（滿版） |
| 面板 | x=118 w=248，右緣 365 | x=99 w=211（66vw），右緣 310 |
| 橫向溢出 | 無 | 無 |
| 觸控目標 <44px | 無 | 無 |
| 開始區與面板重疊 | 無 | 無 |

**這些都還是模擬。** safe-area inset 在模擬下是 0，原生控制項的度量也不對，
所以下面的檢查單仍然要在真機上跑一次。

---

## 離線可用性：星表的網址不加日期版本參數（2026-09-09 修）

`loadCatalog()` 原本在網址後面接一個 `YYYY-MM-DD` 的版本參數，抄的是 `/market/` 與
`/coupon/` 的手法。那個手法對「一天更新好幾班的 feed」是對的，**對星表是錯的**：

- 星表是 J2000 平位置，只有 `build-sky-catalog.mjs` 重跑時才會變，不是每天變。
- 星表在 `/sky/data/` 底下、**不是** `/data/`，所以走的不是 `sw.js` 的 network-first
  分支，而是 **cache-first 靜態資產**分支。
- 星表**刻意沒進 `PRECACHE`**（220 KB 不該讓只看 `/stocks/` 的訪客吞下去，
  `sw.js` 那條註解說得對）。

三件事加起來：每天換一個 cache key ＝ 隔天必定 miss ＝ 沒訊號時整頁不能用。
**而觀星發生的地方正好就是沒訊號的地方。**

實測（清空快取、關掉本機伺服器，用 CDP 量；把 `Date` 換掉模擬隔天）：

| | 修之前 | 修之後 |
|---|---|---|
| 同一天、沒訊號 | ok count=5080 | ok count=5080 |
| **隔一天、沒訊號** | **FAIL: Failed to fetch** | ok count=5080 |

**量這件事有兩個坑，兩個我都踩了**：CDP 的 `Network.emulateNetworkConditions`
只作用在頁面 target，**管不到 service worker 自己發的 fetch**，所以第一次量到
「離線也 200」是假訊號；要真的把伺服器關掉。第二次又因為前一輪的探測已經把
各種網址都灌進快取，量到的仍是假訊號；每一輪都要先清 `caches` 並反註冊 SW。

改法同 `CLAUDE.md` 給 `/float/` 的那條：網址固定，**星表重新產生時 bump `sw.js` 的
`VERSION`**，靠 `activate` 的清理把舊 cache 丟掉。靜態契約釘的是**呼叫點**
（`fetchImpl(CATALOG_URL,` 必須逐字如此），不是檔名後面有沒有問號——第一版釘成後者，
因為網址是常數、問號在別處串上去，那條正則永遠不會命中，反向測試當場抓到。

---

## 中文星名：人工表 + 不出貨的 oracle（2026-09-09）

`sky/data/star-names-zh.json` 是這個 repo 第三份人工維護的 feed（另兩份是
`data/coupons.json` 與 `data/floats.json`）。範圍是 **Vmag ≤ 2.5 且 BSC5 有西方專名的
80 顆**——疊加層最多只顯 8 個標籤而且挑最亮的，所以這個範圍涵蓋畫面上真的會被標到的星。

**為什麼不直接用現成的彙編**（實測 2026-09-09）：

| 來源 | 結果 |
|---|---|
| Wikidata（CC0，授權最理想） | **連不到**（同 `CLAUDE.md` 記的那批被拒網域） |
| Stellarium `chinese` skyculture | 通，3,072 筆，但**授權是 CC BY-SA 4.0** |
| 上游 BSC5 有沒有 HIP 欄位 | **沒有**（只有 HR／HD／SAO／DM／ADS） |

CC BY-SA 正是這個 repo 當初否決 HYG 的同一個理由（「會讓這個 repo 出現第一份帶分享
相同條款的資料」）。而且那份是**簡體**，鍵是 **HIP**——我們的星表沒有 HIP，要經由
`skycultures/modern` 的「IAU 專名→HIP」對照表才 join 得起來。

所以：**名稱自己寫，Stellarium 只當本機核對用的 oracle，一個位元組都不進 repo**
——同 pyerfa 與 HYG 那個既有模式。星名本身是儀象考成一系古籍的公有領域名稱，
受著作權保護的是他們的彙編，不是名稱本身。

核對結果（`scripts/sky-name-audit.mjs`）：80 筆中 **77 筆逐字一致**（扣掉繁簡字形差異
之後），3 筆不同——1 筆只是數字寫法（廿二／二十二）＝同一顆星，記進 `variants` 後保留；
**2 筆是真的分歧（侯／候、尾宿七／尾宿六），值清成 `null`**，畫面自動回退成西方專名。
那條「來源分歧就留 null」是照 `/float/` 7B／8B 的既有慣例，`sky-star-names.test.js`
把它釘成雙向的：標成 conflicting 就不准出貨值，沒有值就必須標成 conflicting。

**這張表的定義性檢查是「鍵指到對的那顆星」**（對應 floats 的
`loadFromShot === shots[].grams`）：HR 抄錯了，畫面就會把某顆星標成另一顆星的名字，
而且肉眼完全看不出來。測試因此要求每個 HR 都存在於星表、且**必須是星表已經有西方
專名的那 334 顆之一**，並逐欄比對抄過來的 `en`／`bayer`。

**繁體那條檢查是黑名單，不是通用驗證器**：唯一的汙染途徑是「核對時把 oracle 的簡體
字串貼回來」，所以列的是那份來源的星名裡真的出現過、且與繁體不同形的 52 個字。
反向測試要拿那張表裡的字（例如「娄」）去試——第一版拿「简」去試，它不在汙染途徑上，
測試當然不會紅，害我一度以為那條有效。

---

## Phase 5：月亮與方位校正（2026-09-09）

### 為什麼是月亮

這一頁最大的誤差來源不是天文計算（殘差 < 0.01 度），是 heading —— 典型偏 2 到 10 度。
`geomag.mjs` 查表一律回 `null`（NOAA 連不到），所以未校正時方位角就是感測器的磁北值。
月亮是天上**唯一一個每個人都叫得出名字**的東西，讓使用者指出它在哪，就等於當場實測
出這條管線的偏差 —— 比查不到的模型直接。

### 月亮**不**拿來校正視野角

視野角是尺度參數，單一個標記只給得出指向偏移。拿月亮的視直徑當尺也不行，實算：

| 方法 | 螢幕上的量（65° 視野、375px 寬） | 視野角差 10% 時的位移 |
|---|---|---|
| 對月亮圓面（視直徑 32.1′） | 直徑 3.1 px | 約 0.3 px（看不出來） |
| 對離軸 15° 的星 | 距中心 79 px | 約 9 px |

差約 30 倍。**視野角仍然走原本的圖案比對，Phase 5 沒有動它。**

### 站心（topocentric）修正不可省

月亮的地平視差**約 1.006 度**（實測基準值算出來的最大值），接近它自己視直徑的兩倍。
拿地心座標直接畫上去，標記最多偏一整度 —— 使用者會把**月球視差**當成**方位角偏差**
校進去，校正結果本身就是錯的。`topocentricEquatorial()` 走 Meeus 第 40 章。

### 那個偏移**不是磁偏角**

拖月亮量到的是**整條 heading 管線的觀測偏差**：地磁偏角、手機磁力計的硬鐵／軟鐵誤差、
感測器融合殘差全混在裡面，分不開也不該假裝分得開。所以：

- 儲存鍵叫 `bjkw-sky-heading-offset`，**不叫 declination**；靜態契約擋住
  `declinationDeg` 這類欄位名。
- `geomag.mjs` 維持回 `null`，兩者在資料模型上是分開的東西。
- 頁面文案明寫「它不等於磁偏角，本站也不會這樣宣稱」，靜態契約釘住那一句。

符號約定（推導寫在 `nextHeadingOffsetDeg` 的註解裡）：畫面刻度是 `rawAz + O`，
使用者說真月亮在我們稱為 `tapAz` 的方向、而它實際在 `moonAz`，
所以 **`O' = O + (moonAz − tapAz)`**，且會累加而不是取代（第二次校正是在第一次上微調）。

### 驗證：oracle 驗的是抄寫，不是絕對精度

`erfa.moon98` 的文件寫著它是 *"a full implementation of the algorithm published by Meeus
... except that the light-time correction to the Moon's mean longitude has been omitted"*
—— 跟 `sky/lib/moon.mjs` **是同一套** Meeus 截斷 ELP2000-82B。所以這組比對驗的是
**120 個週期項有沒有抄錯**，不是演算法的絕對精度。要引絕對精度就引 ERFA 自己的數字：
對現代的 ELP/MPP02，1950–2100 RMS 2.9 角秒、最差 18.3 角秒。

實測殘差：

| 比對 | 殘差 | 說明 |
|---|---|---|
| 地心方向 vs `moon98` | **0.83 角秒（常數）** | 就是 moon98 拿掉的光行時項（1.26 s × 0.549″/s ≈ 0.7″） |
| 地心距離 vs `moon98` | **< 1 km** | Σr 那組係數是獨立的一欄 |
| 站心 vs 向量相減 | **10.5 角秒** | 基準值走 `erfa.pvtob` 直接相減，**與 Meeus 40 是兩條路** |

**踩過的坑：時間尺度。** 第一版把 UTC 直接餵進 Meeus 的式子，但那些式子要的是 TT。
月亮每秒走 0.549 角秒，TT−UTC = 69.184 秒就是 **38 角秒的系統性落後** —— 實測
「黃經固定偏 −38.3 角秒、散佈只有 ±5 角秒，而黃緯幾乎為零」，一眼看得出是時間偏移
而不是係數抄錯（係數錯會兩維一起亂）。修正後降到 0.83 角秒。
ΔT 用常數，下次閏秒後會差 1 秒 = 0.55 角秒，可以忽略但會慢慢過期。

### 門檻是量出來的

`sky-moon.test.js` 的 1.2 角秒門檻不是猜的。逐項把程式改壞、量殘差：

```
基準                        0.83″     最後一位打錯    0.85″  （擋不住，但那只有 0.036″）
數字顛倒                    1.45″     漏掉離心率 E    1.59″     加項打錯      1.50″
漏一位數                    4070″     符號寫反        1311″     漏掉整個週期項 2347″
```

門檻取 1.2 角秒才擋得住中間那三個「subtle」的。**第一版寫 3 角秒，那三個全部漏掉**，
反向測試當場抓到。基準殘差是確定性的常數（同演算法、同一個被拿掉的項），不是浮動雜訊，
所以 45% 的餘裕夠用。

### 順手修掉的一個沒有徵兆的 bug

`hidden` 屬性靠 UA 樣式表的 `display:none` 生效，**任何帶 class 的 display 宣告特異度都
比它高**（`.btn` 是 `inline-flex`）。實測「清除校正」在沒有校正時照樣顯示。
已統一加 `[hidden]{display:none !important}`。這個坑之前為 `.stage-dock` 個別補過一次，
這次才發現該一次擋掉。

---

## 授權提示：哪些是我們的、哪些是 iOS 的（2026-09-10）

使用者回報「同一個分頁重新整理就會重新要求授權」。先把責任分清楚：

**我們這邊查過、沒問題的**：`sw.js` 只有 `skipWaiting`／`clients.claim`，**不會 reload**，
所以不是頁面被載入兩次；`requestPermission()` 的用法與呼叫時機都正確（已授權時會直接
resolve，不會多跳提示）。

**iOS 的部分，頁面壓不下來**：方向／動作的授權是綁在**單一份 document** 上的，重新整理
就是新的一份，所以會重問；相機同理。這不是這個 repo 的 bug，任何網站都一樣。
**這一段我沒有辦法在開發環境驗證**（這裡沒有 iOS Safari），所以頁面文案寫成
「可以試」而不是「這樣就好」，等真機回報再改口。

**位置那一個提示是我們消得掉的，而且已經消掉了。**

### 消掉的方式，以及第一版為什麼沒有真的消掉

這一頁需要位置只是為了算地方恆星時與地平座標，精度需求極低：經度差 0.1 度（約 10 km）
＝ 天球差 0.1 度，遠小於這一頁 2 到 10 度的 heading 誤差。所以把上一次的座標存進
`localStorage`（`bjkw-sky-last-fix`）重整時直接沿用。

**但只做到「快取座標、照樣呼叫 watchPosition」是不夠的** —— 呼叫本身就會跳提示。
第一版就是這樣，用 CDP 假造 geolocation 實測，第二次載入仍然叫了一次 `watchPosition`。
真正有效的是**夠新就完全不呼叫**：`FIX_REFRESH_AFTER_MS = 30 分鐘`。
30 分鐘內就算開了車，位移頂多幾十公里 ＝ 天球上零點幾度，不重新定位是安全的。

實測（CDP，假造 `navigator.geolocation` 計數）：

| 情境 | watchPosition | clearWatch |
|---|---|---|
| 沒有快取（第一次用） | 1 | 1 |
| **剛存過（重新整理）** | **0**（＝不跳提示） | 0 |
| 快取 45 分鐘前 | 1 | 1 |
| 快取 13 小時前（超過 12 小時上限） | 1 | 1 |

### 順帶修掉的真 bug：`watchPosition` 從來沒有 `clearWatch`

`requestLocation()` 原本呼叫 `watchPosition` **連 watch id 都沒接住**，所以頁面開著就
持續定位 —— **觀星時最不該發生的耗電**，而且完全沒有徵兆；錯誤路徑關掉相機之後它還在跑。
現在接住 id，拿到第一筆定位就停（這一頁不需要持續追蹤），`stopCamera()` 也一併清掉。
靜態契約釘住「有接住 id」與「有呼叫 clearWatch」兩件事，兩條都跑過反向測試。

`/weather/` 套同一招（`bjkw-weather-last-fix`），那一頁的位置只用來挑最近的觀測站，
精度需求更低。順便補上這個 repo 其他互動頁都有、只有它漏掉的
`__WEATHER_SKIP_AUTO_INIT__` 與 `window.WeatherApp.helpers`，才測得到。

---

## 控制面板收合成齒輪（2026-09-10）

預設**只有一顆齒輪**，點開才是完整面板。收合態量到 47×46（`summary` 本身 45×44，
過 44px 門檻），展開後 375 寬是 248×427、320 寬是 211×447、橫向 812×375 是 248×355，
三個尺寸都無橫向溢出。

**齒輪用文字字元 `\2699` 畫，不可以用 SVG data URI** —— SVG 需要 xmlns，那個網址會撞到
這一頁「body 內不得出現外部網址」的靜態契約。同 `<select>` 箭頭那個坑。
另加 `font-variant-emoji:text`，免得 iOS 把它渲染成彩色 emoji。

收合成純圖示之後就沒有可見文字了，所以 `<summary>` 必須自己帶 `aria-label`，
靜態契約釘住這件事，也釘住 `skip` link 仍然指得到控制項。

---

## 授權提示：真機推翻了我先前的斷言（2026-09-10）

上一輪我在這裡與 `CLAUDE.md` 寫的是「iOS 重新整理會重問**方向與相機**權限，
那是平台行為」。**真機一測就推翻**：
...
那句是在沒有 iOS 環境的情況下，從「授權綁在 document 上」推理出來的。
**而這個差別不是雞毛蒜皮**：正因為方向權限跨重整還在，回訪自動接續才成立。

另外要分清楚兩件事：使用者把 Safari 網站設定的相機改成「允許」之後，**仍要按
我們自己的「開啟相機與感測器」按鈕**——但那是因為頁面本來就要求按，
不代表系統對話框還會跳。**系統對話框會不會跳，目前仍未確認。**

### 回訪自動接續 `tryAutoResume()`

```
init()
 └─ 旗標 bjkw-sky-autostart === "1"？
      否 → 顯示按鈕（初次訪客一定走這條，不可以偷跑權限）
      是 → 掛 deviceorientation 監聽（純被動）
           └─ 600 ms 內收到帶完整三個角的事件？
                否 → 回退成按鈕：「要重新授權一次」
                是 → 載入 → 星表 → 相機 → 位置 → running
                      相機被拒 → stopCamera() + 回退成按鈕並說原因
```

**唯一合法的探測方式是「掛監聽、看事件有沒有來」。** 掛監聽本身不會跳提示；
而 `requestPermission()` 在使用者手勢之外呼叫會丟 `NotAllowedError`，
自動路徑會整條安靜地失效。靜態契約因此釘住 `tryAutoResume()` 不得出現那個呼叫。

相機那一段是實驗：`getUserMedia` 能不能在沒有手勢下成功，這個環境驗證不了。
成功就省掉一次點擊，失敗就回退成按鈕，不會比原本差。
...
| 沒旗標（初次訪客） | **0** | 顯示按鈕 ← 紅線 |
| 有旗標、方向收不到 | **0** | 「要重新授權一次」 |
| 有旗標、相機被拒 | 1 | 回退成按鈕，訊息說得出原因 |
| 有旗標、全部順利 | 1 | dock 收掉、串流有、視野內 55 顆、旗標留著 |
| 旗標是垃圾值（`"true"`） | **0** | 顯示按鈕 |

「全部順利」那一列是用 Chromium 的 `--use-fake-device-for-media-stream` 拿到
**真的 MediaStream** 才跑得完——自己用 `Object.create(MediaStream.prototype)` 假造的
會在 `srcObject` 那一步被拒。

**量測環境自己的隔離比被測程式更容易騙人。** 五個情境跑在同一個分頁時，
情境 1 的 `setInterval` 汙染了情境 2，量出「方向收不到卻仍呼叫相機」的**假 bug**；
單獨跑情境 2 是對的。改成每個情境開新分頁才穩定。
另外 headless Chromium 在有人掛上 `deviceorientation` 監聽時會送**一筆三個角都是
`null` 的事件**，`readOrientationEvent()` 本來就整筆丟掉，所以探測不受影響，
但數事件數會誤以為「有收到方向」。

### 齒輪去框

收合態是一顆裸齒輪。底色與邊框**明確歸零**（`background:none;border:0`），
不能只是「不寫」——這一頁有一條泛用的 `details` 規則（0,0,1）帶著底色與邊框，
`.hud` 一旦不宣告那條就贏回去，實測量到「面板背景 rgb(14,19,29)、邊框 1px」。
沒了外框那圈對比也沒了，所以收合態的齒輪自己上 `text-shadow` 描邊，
白天的亮畫面上才看得見。命中區維持 51×50。
...
| 9 | **滿版版面**（2026-09-09 新增） | 相機有沒有真的佔滿整個視窗（含瀏海那一側）？右上角面板有沒有被瀏海或狀態列切到——**這一項模擬答不出來**，模擬的 safe-area inset 是 0。 |
| 10 | **拖得動嗎** | 手指從相機畫面上往上拖，捲不捲得到下面的說明與星表？模擬下 `scrollY` 0→439，真機要再確認一次（iOS 的 `touch-action` 行為與 Chromium 不完全一致）。 |
| 11 | **面板的觸控目標** | 「設定與狀態」那條收合列、兩支滑桿。模擬下都是 44px，但滑桿的 thumb 在 iOS 上是原生控制項，度量可能不同——那正是這個 repo 踩過的 `<select>` 那一類。 |
| 12 | **半透明讀不讀得到** | 面板是 `rgba(14,19,29,.55)` + `backdrop-filter`。夜空下沒問題，**白天對著亮背景**時字讀不讀得到？讀不到就要加深或加大 text-shadow。 |
| 13 | **320 寬會不會擋掉半個畫面** | 面板寬度是 `min(248px,66vw)`。在小螢幕上收合後是不是真的只剩一條？ |
| 14 | **離線可用嗎**（2026-09-09 新增） | 開一次 `/sky/` 讓星表進快取，**隔天**到沒訊號的地方再開。星表要載得起來、星點要畫得出來。模擬已驗（見上），真機沒驗過。 |
| 15 | **中文星名有沒有出現、字型對不對** | 北斗七星應該顯示天樞／天璇／天璣／玉衡／開陽／搖光，不是英文。要看的是**有沒有掉成豆腐字**——canvas 的 `ctx.font` 與 body 用同一套字型堆疊，但那是模擬下的結論。Rasalhague 與 Girtab 刻意仍是英文（來源分歧，值留 null），那不是 bug。 |
| 17 | **月亮標記對不對得齊**（Phase 5，未驗） | 月亮在天上、仰角 > 10 度、非新月時，畫面上的橙色圈應該落在真月亮附近（未校正前可能偏 2–10 度，那正是要校的東西）。**這是整個 Phase 5 唯一真正的判準。** |
| 18 | **校正之後星圖有沒有一起正過來** | 點「用月亮校正方位」→ 點真月亮 → 圈應該貼上去，而且**整片星圖要跟著轉同樣的角度**。只有月亮正了、星星沒動，代表偏移沒套進 renderFrame。 |
| 19 | **校正值合不合理** | 台灣的磁偏角約 −4 度（西偏），但量到的是整條管線的偏差，所以看到 ±10 度以內都算合理。**超過 30 度多半是手機磁力計被干擾**（車上、喇叭、磁吸殼），不是程式錯。 |
| 21 | **重整還剩幾個提示**（2026-09-10 新增） | 位置那一個應該不見了（30 分鐘內重整完全不呼叫定位，已用假 geolocation 實測 0 次）。預期只剩方向與相機兩個。 |
| 22 | **Safari 的網站設定能不能讓相機不再問** | 網址列選單 →「網站設定」→ 相機 → 允許。**這條我沒在 iOS 上驗證過**，回報結果之後再決定文案怎麼寫。 |
| 24 | **齒輪好不好按** | 收合態模擬量到 45×44，但那是模擬。真機看命中區、以及齒輪有沒有被渲染成彩色 emoji。 |
| 20 | **不可用的狀態說得清不清楚** | 新月、地平線下、仰角 < 10 度時，按鈕要是灰的而且**說出為什麼**。 |
| 16 | **橫拿時面板會不會擋住開始區** | 實測 720×320 修之前真的重疊，已加 `max-height:480px` 的規則把開始區推到面板左邊那一欄。真機橫拿再看一次。 |
...
2. **「首頁看不到新卡片」的因果我判斷錯了兩次**（更正於 2026-09-09）。
   當時寫的是「`sw.js` 的 `VERSION` 撞號造成首頁變舊」——**那是錯的**。
   `sw.js` 的導覽自 `v7` 起就是 network-first，service worker 端不出舊的 `/`。
   我接著又怪到 CDN 快取，也是錯的，害使用者白清了好幾次快取。
   **真正的病因是 `index.html` 的浮標卡少了 `</div>` 與 `</a>`**（前一次合併衝突把
   共用的結尾連帶吃掉）：瀏覽器自動收掉那個未關的連結，把星空卡重新掛進浮標卡的
   `.entry-foot` 裡，而手機版斷點正好把它隱藏起來——所以症狀是「桌機看得到、
   手機直式看不到、轉成橫向又出現」。已於 PR #18 修好，並補了堆疊式的標籤巢狀檢查。
   VERSION 撞號本身確實會發生（兩條分支同時 bump，git 不判成衝突），但它的代價是
   「舊 cache 不會被清掉」，不是首頁變舊。判準寫在 `CLAUDE.md` 的部署那一節。
   教訓：**「只在某個斷點消失」幾乎一定是結構被瀏覽器重新掛載，快取不會挑寬度**——
   先看渲染後的 DOM，不要一開始就怪快取。
