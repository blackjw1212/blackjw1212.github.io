...
| 2 | 一次點擊拿到所有權限 | iOS 的 `requestPermission` 只能在使用者手勢的呼叫堆疊裡呼叫。按下「開啟相機與感測器」之後，相機、方位、位置是否全部到位？狀態列有沒有出現「還缺：…」？ |
...
| 8 | 分頁切走 | 已知行為：相機不會停、指示燈仍亮。**刻意不處理** —— 停掉 track 之後重開可能在 iOS 觸發再次授權，那是這個環境驗證不了的風險。確認這個行為可以接受，或決定要改。 |
