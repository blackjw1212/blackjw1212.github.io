...
2026-09-03 全站 13 頁在 375 與 320 都已歸零。2026-09-23 複審擴到 **16 條路由＋16 個分頁**
（含之後才加的 `/bait/` 4 分頁、`/float/` 3 分頁、`/sky/`），375／320 共 56 個狀態仍是 0。
要再補時照下面幾條，不要每頁自己發明。
（**那次複審是行動裝置模擬，不是真機。** `/bait/`、`/float/`、`/sky/` 至今沒上過真機——
真機才抓得到的 `<select>` 那一類，只能用「畫面上看得到的 select 全是 `appearance:none`」
間接確認沒有復發，那不能取代實測。`/sky/` 只有 range 與 button、刻意沒有 `<select>`。）
...
- **禁詞是兩份不同的清單，不是一份聯集**（2026-09-19 核對過，先前這裡寫成一份是錯的）：
  首頁禁 `保證`、`可放心`、`買進`、`賣出`、`投資建議`、`安全資訊`——`買進`／`賣出`
  是**裸字**就禁，不限定「訊號」；`/market/` 禁 `保證`、`可放心`、`買進訊號`、`賣出訊號`、
  `實領淨收益`——**不含 `投資建議`**，因為同頁被釘住的揭露句「不是投資建議」本身就含
  這個子字串，禁了會自打。改任一頁文案前對照它自己那條 `assertNoMatch`，不要看另一頁的。
  **註解也算**——見下面「註解也算」那一節。
...
- **`confidence` 有四個值，而且與 `variants[]` 正交。** `cross-checked`（≥2 個來源給同一個
  數字，測試會檢查 `sourceIds.length >= 2`）／`single-source`／`conflicting`（來源分歧）／
  `off-scale`。後兩者都**值留 `null`**、把看到的數字記進 `variants`、而且必須寫 `note`。
  廠牌差異走 `variants[]`，不塞進 confidence——「這個數字有多可信」與「別家給的是多少」
  是兩件事。
- **`off-scale` 與 `conflicting` 不一樣，別混用。** `conflicting` 是「多個來源互相吵架」；
  `off-scale` 是「**唯一**說話的那個來源，我們已經在同系列的別的級距上不採信他」。
  這個標籤**必須自己賺到**：測試要求那個來源真的在同系列的**其他 ≥3 列**被降級成
  `variants`，沒有那些紀錄就不准用——否則它會退化成「我不喜歡這個來源」的萬用藉口。
  現況只有 `G10` 是 `off-scale`：它的唯一來源 `jp-deraturi` 的 G 系在本表已有六級
  被降級且系統性偏輕，而同一個理由早就被用來拒絕該來源的 7B／8B。**那原本是資料內部
  的雙重標準**，2026-09-17 的審查抓出來的。
...
- **資料裡給人看的文字，每一個欄位都必須被頁面讀到。** 沒被渲染的散文是死的，
  而且更糟——維護的人會以為自己在改畫面上的東西。2026-09-17 的審查一口氣找到六段
  （`scope`、`residualBuoyancy.emptyHookTarget` 與 `.note`、`mainSinker.note`、
  `depthPolicy.allowedWhen` 與 `.note`），全部沒有任何讀者看得到。
  `float-schema.test.js` 有一條**會自動抓新欄位**的檢查：走訪整份資料，凡是值為字串、
  長度 ≥12、且欄位名不在 `INTERNAL_FIELDS` 允許清單裡的，欄位名都必須出現在頁面的
  行內 script 中。新增一個給人看的欄位而忘了渲染就當場紅。要豁免就把欄位名加進
  那份清單並在旁邊寫理由。
- **三條資料健康度的閘門**（同樣是那次審查補的，先前都沒有測試）：
  相鄰級距比值必須落在 1.15〜1.60（`G10` 當初的 ×2.33 就是被這條抓到的，
  上限 1.60 是為了容得下 B 系 4B→5B 的 ×1.54）；標 `nominal` 的号数必須真的等於
  `parseFloat(label) × 3.75`（容差 0.01，擋「填了實測值卻還標 nominal」）；
  只靠單一來源的列不得超過三成（2026-09-19 實算 9/44 ＝ 20%——這個數字會隨資料變，
  別把它當成常數抄；這是防滑坡的粗閘門不是精確門檻）。
- **新增或修改這幾條檢查之前，先跑一次反向測試。** 不會咬人的檢查沒有價值，這一頁
  已經為此驗過兩輪：深度不變式（塞「沉入 3.2 cm」）、以及上面五條（把 G10 的值填回去、
  `off-scale` 指向沒被降級的來源、`nominal` 填實測值、塞一個跳級的值、加一段不渲染的
  文字）——五條全部確認會紅，改完再還原。
...
- **月亮（Phase 5）走自己的路徑，不碰恆星那條。** `sky/lib/moon.mjs` 是 Meeus 47 的
  ...
  省掉會讓使用者把視差當成方位偏差校進去；**(2) 星曆的時間引數要 TT 不是 UTC** ——
  差 69.184 秒 ＝ 38 角秒的系統性落後，實測就是「黃經固定偏 −38″ 而黃緯為零」；
  **(3) 月亮不能拿來校視野角** —— 視直徑在 65 度視野下只有 3.1 px，視野角差 10%
  才動 0.3 px，比用星圖圖案差約 30 倍。
- **拖月亮量到的偏移不是磁偏角**，是整條 heading 管線的觀測偏差（磁偏角 + 磁力計
  硬鐵軟鐵 + 融合殘差）。儲存鍵是 `bjkw-sky-heading-offset`，靜態契約擋住
  `declinationDeg` 這種命名，`geomag.mjs` 仍然回 `null`。同「不可以用 0 代替未知」那條紅線。
- **驗月亮的 oracle 只驗抄寫，不驗絕對精度**：`erfa.moon98` 實作的是同一套 Meeus 截斷
  （它自己的文件寫的），所以 0.83 角秒的殘差只代表 120 個係數沒抄錯。絕對精度要引
  ERFA 的數字（對 ELP/MPP02，RMS 2.9″、最差 18.3″）。站心那一段的基準值刻意改用
...
- **iOS 重整之後只有相機會重問，方向／動作權限跨重整仍然有效**（真機實測 2026-09-10）。
  **我先前在這裡寫的是「方向與相機都會重問」，那是錯的** —— 沒有 iOS 環境、憑
  「授權綁在 document 上」推理出來的，真機一測就推翻。這條差別不是雞毛蒜皮：
  **正是因為方向權限還在，回訪自動接續才成立**。
  Safari 網站設定把相機改成「允許」之後**系統對話框還會不會跳，仍未確認** ——
  上一輪測的是「還要不要按我們自己的按鈕」，那是兩件事，不要混為一談。
  位置那一個消得掉：夠新的座標就**完全不呼叫定位**（`FIX_REFRESH_AFTER_MS`
...
- **回訪自動接續（`tryAutoResume`）**：成功開過一次才寫旗標
  （`bjkw-sky-autostart`，只認字串 `"1"`），之後再開就先默默試。
  **探測方式只有一種是合法的：掛監聽、看事件有沒有來。** 掛監聽本身不會跳任何提示；
  而 `requestPermission()` 在使用者手勢之外呼叫會丟 `NotAllowedError` 且是安靜地壞掉
  ——靜態契約釘住 `tryAutoResume()` 不得出現那個呼叫。
  **絕對不可以對初次訪客自動開相機**（沒有旗標就顯示按鈕），這條有測試守著。
  CDP 五個情境都實測過：沒旗標／方向收不到／相機被拒／全部順利／旗標是垃圾值。
  相機能不能在沒有手勢下拿到，**這個環境驗證不了**，失敗就回退成按鈕，不會更差。
- **量這種東西時，測試環境自己的隔離比被測程式更容易騙人。** 五個情境跑在同一個
  分頁時，情境 1 的 `setInterval` 汙染了情境 2，量出「方向收不到卻仍呼叫相機」的
  假 bug；單獨跑情境 2 是對的。改成**每個情境開一個新分頁**才穩定。
  另外 headless Chromium 在有人掛上 `deviceorientation` 監聽時會送**一筆三個角都是
  `null` 的事件**——`readOrientationEvent()` 本來就會整筆丟掉，所以探測不受影響，
  但看事件數會誤以為「有收到方向」。
- **`watchPosition` 一定要 `clearWatch`。** `/sky/` 原本連 watch id 都沒接住，頁面開著
  就持續定位——觀星時最不該發生的耗電，而且完全沒有徵兆。靜態契約釘住「有接住 id」
  與「有呼叫 clearWatch」。
- **右上角面板收合態是「一顆裸齒輪」，沒有外框。** 底色與邊框要**明確歸零**
  （`background:none;border:0`），不能只是「不寫」——這一頁有一條泛用的
  `details` 規則（0,0,1）帶著底色與邊框，`.hud` 一旦不宣告那條就贏回去，
  實測量到「面板背景 rgb(14,19,29)、邊框 1px」。沒了外框那圈對比也沒了，
  所以收合態的齒輪要自己上 `text-shadow` 才在白天的亮畫面上看得見。
- **齒輪用文字字元 `\2699` 畫，**
...
**`0bbc64dd` 是這頁版面的穩定基線**（2026-09-24，經兩輪外部版面審查後定案）。
下面每一條都是量過之後的取捨，**不是待修的 bug**。之後動版面只針對新的具體問題；
遇到這幾條，先說它是已接受的取捨，不要主動再調——調其中一條會把另一條已解決的問題打開。

- **≥1280 三欄比例是 `1.1 / .8 / 1.0`（天氣／潮汐／海況）。** 三塊各自要的寬度實測是：
  逐時預報 7 格不橫捲約 546px、海況一列不斷行約 453px、潮汐 2×2 最少 270px。
  1440 寬時三欄合計只有 1305px，放不下三個。把潮汐壓到 284px 能讓前兩者都達標，
  但潮窗與潮況斷行、潮汐卡變高，天氣與海況下方各空出 167／288px——那正是使用者
  最早抱怨的「下方空白」。這組比例在 1440 空白合計 80px、逐時溢出 51px；
  1600 以上三欄共 1480px，全部放得下。**審查建議過 1.15/1/0.95，那會讓海況欄更窄，別照做。**
- **中欄潮汐固定 `repeat(2, …)`。** 原本的 `auto-fit, minmax(150px, 1fr)` 在窄欄會
  退成單欄，卡片高度暴增。
- **1440 寬時逐時預報橫捲約 51px 是接受的。** 不要為了消它去縮字或把間距壓到極限。
- **1920 寬時海況卡下方約 131px 空白是接受的。** 海況內容本來就比較少，不強制等高。
- **不用 `zoom`／`transform: scale` 整頁放大。** 試過（≥1840 `zoom:1.125`、≥2040
  `zoom:1.25`）後拿掉，改成**分層字級**：≥1280／≥1600 只放大決策結果、核心數值、
  第二層區塊標題；輔助資料維持、註記縮到 12px；第三、四層卡片（未來 3 天、測站、
  明後天）收掉陰影、底色降一階。
- **768–1279 是兩欄，排法是「天氣｜海況」並排，其餘各佔滿版一列。** 先前測站疊在
  海況下方，天氣卡旁邊空出 494px。
- **「未來 3 天」是 72 小時窗口，不是 3 個日曆日**，所以常跨到第 4 個日期
  （從 06:00 起算時是 24 格、最後一格是第 4 天的 03:00）。前三天標今天／明天／後天，
  **第 4 天不標**——不要為了看起來像 3 天而錯標日期。
- **`.hz-scroll` 設了 `scrollbar-width` 之後，Chrome 就不讀 `::-webkit-scrollbar` 的顏色**，
  會退回瀏覽器預設的淺色軌道。主題色要靠 `scrollbar-color`。

這頁**本機驗不到真資料**：Worker 的 CORS 只放行 `https://blackjw1212.github.io`，
從 `localhost` 打 proxy 會被擋、頁面只剩錯誤態。做法是在 Node 端（不受 CORS 限制）
把 proxy 的回應抓成 `weather/_fixtures/*.json`，頁面載入後 stub `window.fetch` 導到
那些檔，再手動呼叫 `loadAll()`。**要抓齊 22 個縣市的鄉鎮預報**——只抓一個縣市的話，
GPS 永遠只比對得到那個縣市的鄉鎮，「依定位挑海域」的前兩層根本測不到。
`_fixtures/` 測完一定要刪，不能 commit。

其他這頁特有、會害人重做的事：

- **陣風幾乎拿不到。** `O-A0001-001` 實測 876 站只有 39 站有 `PeakGustSpeed`，其餘是 `-99`，
  而且集中在北部與屏東。沒回報就不顯示，**不要去抓幾十公里外的站充數**（布袋最近的
  有陣風站是 76km 外的七美）。
- **不推估即時潮位。** 官方只給滿乾潮時刻與潮高，沒有連續潮位；頁面只說得出漲／退與
  距下個極值多久。同「不要為了 UI 完整性製造不可驗證的連續數值」那條。
- **警特報的「生效中」格式沒有實測過。** 接上時全台沒有任何特報、颱風只有一則已解除的
  舊資料。`W-C0033-001` 的 hazard 結構取自兩個獨立的開源客戶端
  （`kerkerj/cwa-weather`、`breezy-weather`），不是活資料。下一次有強風特報或颱風時要
  親眼看一次橫幅與初判降級。`W-C0033-001` 的類別**沒有「海上強風」**，只有陸上強風。
- **截圖做不到時的驗證方式**：瀏覽器窗格隱藏時截圖會逾時，版面數字全靠
  `getBoundingClientRect()` 量；也因此 `requestAnimationFrame` 不會觸發，潮汐曲線要
  手動呼叫 `syncTideCurves()` 才會畫——那是環境限制，不是程式壞掉。
